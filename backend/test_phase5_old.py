"""
Phase 5 Test Suite - Fuzzy Matching & Deduplication
Tests for enhanced fuzzy matcher, Civil ID validation, deduplication, and screening
"""
import sys
import os
...
def test_civil_id_validation():
    ...
    # Valid Civil ID (Y=2, MM=08, DD=14, SSSSS=12355, C=1)
    valid_id = "208141235512"
    result = validator.validate_civil_id(valid_id)
    print(f"\nValidating {valid_id}: {result}")
    assert result['valid'], "Valid Civil ID should pass"
    assert result['normalized'] == valid_id, "Should be normalized correctly"
    
    # Invalid length
    invalid_id = "12345"
    result = validator.validate_civil_id(invalid_id)
    print(f"Validating {invalid_id}: {result}")
    assert not result['valid'], "Invalid length should fail"
    
    # Invalid month
    invalid_month = "213081412355"  # Month = 13 (3rd and 4th digits)
    result = validator.validate_civil_id(invalid_month)
    print(f"Validating {invalid_month}: {result}")
    assert not result['valid'], "Invalid month should fail"
    
    # Invalid day
    invalid_day = "203321412355"  # Day = 33
    result = validator.validate_civil_id(invalid_day)
    print(f"Validating {invalid_day}: {result}")
    assert not result['valid'], "Invalid day should fail"


def test_civil_id_normalization():
    """Test Civil ID normalization with different formats"""
    validator = CivilIDValidator()
    ...
    print("\nNormalization tests:")
    for civil_id in formats:
        normalized = validator.normalize_civil_id(civil_id)
        print(f"{civil_id:20s} → {normalized}")
        assert normalized == "208141235512", f"Should normalize to 208141235512"
...
def test_civil_id_extraction():
    ...
    civil_id = "208141235512"
    info = validator.extract_info(civil_id)
    
    print(f"\nExtracted info from {civil_id}:")
    print(f"  Year digit: {info['year_digit']}")
    print(f"  Month: {info['month']}")
    print(f"  Day: {info['day']}")
    print(f"  Serial: {info['serial_number']}")
    print(f"  Check digit: {info['check_digit']}")
    
    assert info['year_digit'] == '2', "Year digit should be 2"
    assert info['month'] == '08', "Month should be 08"
    assert info['day'] == '14', "Day should be 14"
...
def test_civil_id_duplicates():
    ...
    civil_ids = [
        "208141235512",
        "310151612456",  # Valid: Y=3, MM=10, DD=15
        "208-141235-512",  # Duplicate of first
        "194101612567",   # Valid: Y=1, MM=94... invalid month
        "310151612456"     # Duplicate of second
    ]
    ...
    # Should find at least 2 sets if all are valid, or 1 set if some are invalid
    assert len(duplicates) >= 1, "Should find at least 1 set of duplicates"
    assert "208141235512" in duplicates, "Should find first duplicate"
...
def main():
    """Run all tests"""
    print("="*80)
    print("PHASE 5 TEST SUITE - FUZZY MATCHING & DEDUPLICATION")
    print("="*80)
    ...
    runner.run_test("Test 6: Civil ID validation", test_civil_id_validation)
